...
class LoggerData extends RefCounted:
	...
	## Get all entries.
	func get_entries(min_level := Log.Level.FRAME_ONLY) -> Array[LogEntry]:
...
func _build_view_trees() -> void:
	...
	for module_id in module_ids:
		...
		var module_node = LogNode.new(
			...
			LogEntry.new(Log.Level.SILENT, module_id, "", null)
		)
	...
	if empty_modules != "":
		module_root.add_child(LogNode.new(
			...
			LogEntry.new(Log.Level.INFO, "", "Modules with no logs to display:\n" + empty_modules, null)
		))
