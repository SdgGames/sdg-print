...
@export var collapse_level: Log.Level = Log.Level.INFO:
	set(value):
...
func load_dump_file(path: String):
	...
	_current_dumps = await ErrorDump.load_dumps(path)
...
func _setup_entry_item(item: TreeItem, entry: LogEntry) -> void:
	...
	if entry.current_frame != null:
		...
		if entry.level == Log.Level.FRAME_ONLY:
...
func _setup_frame_data(parent: TreeItem, entry: LogEntry):
	...
	parent.collapsed = Log.Level.FRAME_ONLY >= collapse_level
...
func get_level_color(level: Log.Level) -> Color:
	match level:
		Log.Level.ERROR:
...
func get_message_color(level: Log.Level) -> Color:
	match level:
		Log.Level.ERROR:
			return print_settings.error_color
		Log.Level.WARNING:
